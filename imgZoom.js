 ...
 **/
  class ImgZoom {
    ...
    constructor(elImg, opts) {
      const self = this;
      this.elImg = elImg;
      this.pnodeW = elImg.parentNode.offsetWidth;
      this.pnodeH = elImg.parentNode.offsetHeight;
      ...
      this.options.MIN_SCALE = this.options.MIN_SCALE > 1 ? this.options.MIN_SCALE : 1;
    ...
    destroy() {
      this.elImg.removeEventListener('mousewheel', mousewheelCb);
      this.elImg.removeEventListener('mousedown', mousedownCb);
    }
    ...
    calcCenter() {
      var __this = this;
      __this.elImg.style.transitionDuration = '0s';

      var image = new Image();
      image.onload = function() {
        image.onload = null;
        __this.initTranslateX = -__this.elImg.offsetWidth/2;
        __this.initTranslateY = -__this.elImg.offsetHeight/2;
        setTranslate(__this.elImg, __this.initTranslateX, __this.initTranslateY);
        
        setTimeout(()=>{__this.elImg.style.transitionDuration = '0.5s';}, 500);
      }
      image.src = __this.elImg.src;
    }
  }
  ...
  */
  function mousemoveCb(e) {
    ...
    if(imgZoomInst.scale*$dom.offsetWidth > imgZoomInst.pnodeW) {
      if (translateX - imgZoomInst.initTranslateX > canMovePixX) translateX = imgZoomInst.initTranslateX + canMovePixX;
      if (translateX - imgZoomInst.initTranslateX < -canMovePixX) translateX = imgZoomInst.initTranslateX - canMovePixX;
    } else {
      translateX = imgZoomInst.initTranslateX;
    }


    if(imgZoomInst.scale*$dom.offsetHeight > imgZoomInst.pnodeH) {  
      if (translateY - imgZoomInst.initTranslateY > canMovePixY) translateY = imgZoomInst.initTranslateY + canMovePixY;
      if (translateY - imgZoomInst.initTranslateY < -canMovePixY) translateY = imgZoomInst.initTranslateY - canMovePixY;
    } else {
      translateY = imgZoomInst.initTranslateY;
    }
    console.log(`translateX=${translateX}, canMovePixX=${canMovePixX}, imgZoomInst.initTranslateX=${imgZoomInst.initTranslateX}, offsetWidth=${$dom.offsetWidth}`);
